...
SECRET_KEY=("sdadşlakdşla23milm1imşdi23l4k1321")
ALGORITHM="HS256"
def get_db():
    db = SessionLocal()
    try:
...
def create_access_token(username:str,user_id:int,role:str,exprires_delta:timedelta):
    ...
    return jwt.encode(encode,SECRET_KEY,algorithm=ALGORITHM) ## not payload
...
async def get_current_user(token:Annotated[str,Depends(oauth2_bearer)]):
    try:
        ...
        if username is None or user_id is None:
            raise HTTPException(status_code=HTTP_401_UNAUTHORIZED,detail="username or ID is invalid")
